...
Create P/U/N labels from:
  ...
  - optional CDD rpsblast fmt6 (additional non-RdRP conflict filter)
...
3) Non-RdRP Pfam overlap is checked in two places:
   ...
   - overlap between the RdRpCATCH span and non-RdRP Pfam aligned regions
     (conflict evidence)
4) A strong RdRpCATCH hit that mostly overlaps a non-RdRP Pfam/CDD domain and lacks
   strong palm_annot support is kept unlabeled rather than forced negative.
5) Negative labels are reserved for fully clean non-RdRP cases with strong whole-
   sequence non-RdRP coverage and no RdRpCATCH/palm_annot positive evidence.
6) Every FASTA record gets a decision row explaining the evidence and the final
   label.
...
Let:
  ...
  FULL_NEG      := whole-sequence non-RdRP Pfam/CDD union coverage >= --neg-qcov

Final label:
  P if RC_POS and PALM_POS and PALM_SPAN exists and not PALM_CONFLICT
  N if FULL_NEG and not RC_POS and not PALM_POS
  U otherwise
...
If CDD rpsblast results are provided, they are used the same way as Pfam for
negative/unlabeled evidence only; they never create a positive on their own.
...
def normalize_cdd_id(acc: str) -> str:
...
def read_cdd_list(path: str) -> set:
    out = set()
    with open_text_maybe_gzip(path) as f:
        for line in f:
            t = line.strip()
...
class RpsBlastHit:
    ...
    bitscore: Optional[float]
    qstart: Optional[int]
    qend: Optional[int]
    slen: Optional[int]
    title: str
    qcovs: Optional[float]
    qcovhsp: Optional[float]


def parse_rpsblast_fmt6(path: str, logger: logging.Logger) -> Dict[str, List[RpsBlastHit]]:
    """
    Parse tabular rpsblast output in the custom 14-column format used by the sample:
      0 qseqid
      1 sseqid
      2 evalue
      3 bitscore
      ...
      6 sstart
      7 send
      8 <unused by this script>
      9 qlen
     10 slen
     11 stitle
     12 qcovs
     13 qcovhsp
    Only query-side coordinates are used for overlap/coverage.
    """
    out: Dict[str, List[RpsBlastHit]] = defaultdict(list)
    n = 0
    n_bad = 0
    with open_text_maybe_gzip(path) as f:
        ...
        for parts in reader:
            ...
            try:
                ...
                sid = (parts[1] or "").strip()
                if not qid:
                    n_bad += 1
                    continue
                rec = RpsBlastHit(
                    qid=qid,
                    qlen=_coerce_int(parts[9]) if len(parts) > 9 else None,
                    target_id=sid,
                    target_id_norm=normalize_cdd_id(sid),
                    evalue=_coerce_float(parts[2]) if len(parts) > 2 else None,
                    bitscore=_coerce_float(parts[3]) if len(parts) > 3 else None,
                    qstart=_coerce_int(parts[4]) if len(parts) > 4 else None,
                    qend=_coerce_int(parts[5]) if len(parts) > 5 else None,
                    slen=_coerce_int(parts[10]) if len(parts) > 10 else None,
                    title=(parts[11] or "").strip() if len(parts) > 11 else "",
                    qcovs=_coerce_float(parts[12]) if len(parts) > 12 else None,
                    qcovhsp=_coerce_float(parts[13]) if len(parts) > 13 else None,
                )
            except Exception:
                n_bad += 1
                continue
            out[qid].append(rec)
            n += 1
    logger.info("Loaded rpsblast fmt6 rows=%d (bad=%d), unique_qids=%d from %s", n, n_bad, len(out), path)
    return out


def span_from_rpsblast(hit: RpsBlastHit) -> Tuple[int, int]:
    if hit.qstart is None or hit.qend is None:
        return (0, 0)
    return to_half_open_1based(hit.qstart, hit.qend)
...
def pfam_nonrdrp_hit_stats(
    ...
    for h in hits:
        ...
        model = f"PFAM|{h.target_acc_base or h.target_name}|{h.target_name}"
        model_to_intervals[model].append(iv)
        kept_hits.append(h)
    return all_intervals, model_to_intervals, kept_hits


def _short_cdd_model_name(title: str, target_id_norm: str) -> str:
    bits = [x.strip() for x in (title or "").split(",")]
    if len(bits) >= 2 and bits[1]:
        return bits[1].replace("	", " ")
    if bits and bits[0]:
        return bits[0].replace("	", " ")
    return target_id_norm or "CDD"


def cdd_nonrdrp_hit_stats(
    hits: Sequence[RpsBlastHit],
    cdd_rdrps: set,
    neg_evalue: float,
) -> Tuple[List[Tuple[int, int]], Dict[str, List[Tuple[int, int]]], List[RpsBlastHit]]:
    all_intervals: List[Tuple[int, int]] = []
    model_to_intervals: Dict[str, List[Tuple[int, int]]] = defaultdict(list)
    kept_hits: List[RpsBlastHit] = []
    for h in hits:
        if h.target_id_norm in cdd_rdrps:
            continue
        if h.evalue is None or h.evalue > neg_evalue:
            continue
        iv = span_from_rpsblast(h)
        if iv[1] <= iv[0]:
            continue
        all_intervals.append(iv)
        short_name = _short_cdd_model_name(h.title, h.target_id_norm)
        model = f"CDD|{h.target_id_norm}|{short_name}"
        model_to_intervals[model].append(iv)
        kept_hits.append(h)
    return all_intervals, model_to_intervals, kept_hits


def merge_model_interval_maps(*maps: Dict[str, List[Tuple[int, int]]]) -> Dict[str, List[Tuple[int, int]]]:
    out: Dict[str, List[Tuple[int, int]]] = defaultdict(list)
    for mp in maps:
        for k, iv in mp.items():
            out[k].extend(iv)
    return out


def source_string(has_pfam: bool, has_cdd: bool) -> str:
    parts = []
    if has_pfam:
        parts.append("pfam")
    if has_cdd:
...
def decide_record(
    ...
    pfam_intervals, pfam_model_to_intervals, pfam_kept_hits = pfam_nonrdrp_hit_stats(
        pfam_hits, palm_pfams, neg_ie, pfam_span_source
    )
    cdd_intervals, cdd_model_to_intervals, cdd_kept_hits = cdd_nonrdrp_hit_stats(
        cdd_hits, cdd_rdrps, neg_ie
    )
    all_intervals = list(pfam_intervals) + list(cdd_intervals)
    all_model_to_intervals = merge_model_interval_maps(pfam_model_to_intervals, cdd_model_to_intervals)

    pfam_full_cov = (union_len(pfam_intervals) / float(L)) if (L > 0 and pfam_intervals) else 0.0
    cdd_full_cov = (union_len(cdd_intervals) / float(L)) if (L > 0 and cdd_intervals) else 0.0
    full_cov = (union_len(all_intervals) / float(L)) if (L > 0 and all_intervals) else 0.0

    pfam_full_models = summarize_model_coverage(pfam_model_to_intervals, topn=8)
    cdd_full_models = summarize_model_coverage(cdd_model_to_intervals, topn=8)
    full_models = summarize_model_coverage(all_model_to_intervals, topn=8)
    ...
    pfam_rc_overlap_aa = intersect_len(rc_span_iv, pfam_intervals) if rc_span_iv else 0
    cdd_rc_overlap_aa = intersect_len(rc_span_iv, cdd_intervals) if rc_span_iv else 0
    rc_overlap_aa = intersect_len(rc_span_iv, all_intervals) if rc_span_iv else 0

    pfam_rc_overlap_frac = (pfam_rc_overlap_aa / float(rc_span_len)) if rc_span_len > 0 else 0.0
    cdd_rc_overlap_frac = (cdd_rc_overlap_aa / float(rc_span_len)) if rc_span_len > 0 else 0.0
    rc_overlap_frac = (rc_overlap_aa / float(rc_span_len)) if rc_span_len > 0 else 0.0

    pfam_rc_overlap_models = summarize_model_overlap(rc_span_iv, pfam_model_to_intervals, topn=8) if rc_span_iv else ""
    cdd_rc_overlap_models = summarize_model_overlap(rc_span_iv, cdd_model_to_intervals, topn=8) if rc_span_iv else ""
    rc_overlap_models = summarize_model_overlap(rc_span_iv, all_model_to_intervals, topn=8) if rc_span_iv else ""
    ...
    pfam_palm_overlap_aa = intersect_len(palm_span_iv, pfam_intervals) if palm_span_iv else 0
    cdd_palm_overlap_aa = intersect_len(palm_span_iv, cdd_intervals) if palm_span_iv else 0
    palm_overlap_aa = intersect_len(palm_span_iv, all_intervals) if palm_span_iv else 0

    pfam_palm_overlap_frac = (pfam_palm_overlap_aa / float(palm_span_len)) if palm_span_len > 0 else 0.0
    cdd_palm_overlap_frac = (cdd_palm_overlap_aa / float(palm_span_len)) if palm_span_len > 0 else 0.0
    palm_overlap_frac = (palm_overlap_aa / float(palm_span_len)) if palm_span_len > 0 else 0.0

    pfam_palm_overlap_models = summarize_model_overlap(palm_span_iv, pfam_model_to_intervals, topn=8) if palm_span_iv else ""
    cdd_palm_overlap_models = summarize_model_overlap(palm_span_iv, cdd_model_to_intervals, topn=8) if palm_span_iv else ""
    palm_overlap_models = summarize_model_overlap(palm_span_iv, all_model_to_intervals, topn=8) if palm_span_iv else ""

    pfam_full_neg = pfam_full_cov >= neg_qcov
    cdd_full_neg = cdd_full_cov >= neg_qcov
    full_neg = full_cov >= neg_qcov

    pfam_rc_conflict = pfam_rc_overlap_aa >= rc_ov_aa and pfam_rc_overlap_frac >= rc_ov_frac
    cdd_rc_conflict = cdd_rc_overlap_aa >= rc_ov_aa and cdd_rc_overlap_frac >= rc_ov_frac
    rc_conflict = rc_overlap_aa >= rc_ov_aa and rc_overlap_frac >= rc_ov_frac

    pfam_palm_conflict = pfam_palm_overlap_aa >= palm_ov_aa and pfam_palm_overlap_frac >= palm_ov_frac
    cdd_palm_conflict = cdd_palm_overlap_aa >= palm_ov_aa and cdd_palm_overlap_frac >= palm_ov_frac
    palm_conflict = palm_overlap_aa >= palm_ov_aa and palm_overlap_frac >= palm_ov_frac

    full_neg_sources = source_string(bool(pfam_intervals), bool(cdd_intervals)) if full_neg else ""
    rc_conflict_sources = source_string(pfam_rc_overlap_aa > 0, cdd_rc_overlap_aa > 0) if rc_conflict else ""
    palm_conflict_sources = source_string(pfam_palm_overlap_aa > 0, cdd_palm_overlap_aa > 0) if palm_conflict else ""
    ...
    elif full_neg and (not rc_pos) and (not palm_pos):
        label = "N"
        decision_code = "N_NONRDRP_PFAM_OR_CDD_DOMINANT"
    else:
        label = "U"
        if rc_pos and palm_pos and palm_has_span and palm_conflict:
            decision_code = "U_PALM_SPAN_CONFLICTS_WITH_NONRDRP_PFAM_OR_CDD"
            ...
            decision_code = "U_RDRPCATCH_CONFLICTS_WITH_NONRDRP_PFAM_OR_CDD"
        elif full_neg and (not palm_pos) and rc_pos:
            decision_code = "U_FULL_NONRDRP_BUT_RDRPCATCH_POSITIVE"
    ...
    label_rec = {
        ...
        # legacy compatibility fields now summarize the combined non-RdRP evidence
        ...
        "pfam_neg_qcov_max": _fmt_float(pfam_full_cov, 4),
        "cdd_neg_qcov_max": _fmt_float(cdd_full_cov, 4),
        "pfam_rdrpcatch_overlap_aa": str(int(pfam_rc_overlap_aa)) if rc_span_iv else "0",
        "pfam_rdrpcatch_overlap_frac": _fmt_float(pfam_rc_overlap_frac, 4) if rc_span_iv else "",
        "pfam_palm_overlap_aa": str(int(pfam_palm_overlap_aa)) if palm_span_iv else "0",
        "pfam_palm_overlap_frac": _fmt_float(pfam_palm_overlap_frac, 4) if palm_span_iv else "",
        "cdd_rdrpcatch_overlap_aa": str(int(cdd_rc_overlap_aa)) if rc_span_iv else "0",
        "cdd_rdrpcatch_overlap_frac": _fmt_float(cdd_rc_overlap_frac, 4) if rc_span_iv else "",
        "cdd_palm_overlap_aa": str(int(cdd_palm_overlap_aa)) if palm_span_iv else "0",
        "cdd_palm_overlap_frac": _fmt_float(cdd_palm_overlap_frac, 4) if palm_span_iv else "",
        "nonrdrp_full_negative_sources": full_neg_sources,
        "nonrdrp_rdrpcatch_conflict_sources": rc_conflict_sources,
        "nonrdrp_palm_conflict_sources": palm_conflict_sources,
    }
    ...
    decision_rec = {
        ...
        "full_nonrdrp_negative_sources": full_neg_sources,
        "pfam_full_nonrdrp_negative": "1" if pfam_full_neg else "0",
        "cdd_full_nonrdrp_negative": "1" if cdd_full_neg else "0",
        "rdrpcatch_overlap_conflict": "1" if rc_conflict else "0",
        "rdrpcatch_overlap_conflict_sources": rc_conflict_sources,
        "pfam_rdrpcatch_overlap_conflict": "1" if pfam_rc_conflict else "0",
        "cdd_rdrpcatch_overlap_conflict": "1" if cdd_rc_conflict else "0",
        "palm_overlap_conflict": "1" if palm_conflict else "0",
        "palm_overlap_conflict_sources": palm_conflict_sources,
        "pfam_palm_overlap_conflict": "1" if pfam_palm_conflict else "0",
        "cdd_palm_overlap_conflict": "1" if cdd_palm_conflict else "0",
        ...
        "pfam_neg_domain_count": str(len(pfam_kept_hits)),
        "pfam_neg_union_cov": _fmt_float(pfam_full_cov, 4) if pfam_kept_hits else "",
        "pfam_neg_models": pfam_full_models,
        "pfam_rdrpcatch_overlap_aa": str(int(pfam_rc_overlap_aa)) if rc_span_iv else "0",
        "pfam_rdrpcatch_overlap_frac": _fmt_float(pfam_rc_overlap_frac, 4) if rc_span_iv else "",
        "pfam_rdrpcatch_overlap_models": pfam_rc_overlap_models,
        "pfam_palm_overlap_aa": str(int(pfam_palm_overlap_aa)) if palm_span_iv else "0",
        "pfam_palm_overlap_frac": _fmt_float(pfam_palm_overlap_frac, 4) if palm_span_iv else "",
        "pfam_palm_overlap_models": pfam_palm_overlap_models,
        "cdd_negative_enabled": "1" if cdd_rdrps else "0",
        "cdd_neg_domain_count": str(len(cdd_kept_hits)),
        "cdd_neg_union_cov": _fmt_float(cdd_full_cov, 4) if cdd_kept_hits else "",
        "cdd_neg_models": cdd_full_models,
        "cdd_rdrpcatch_overlap_aa": str(int(cdd_rc_overlap_aa)) if rc_span_iv else "0",
        "cdd_rdrpcatch_overlap_frac": _fmt_float(cdd_rc_overlap_frac, 4) if rc_span_iv else "",
        "cdd_rdrpcatch_overlap_models": cdd_rc_overlap_models,
        "cdd_palm_overlap_aa": str(int(cdd_palm_overlap_aa)) if palm_span_iv else "0",
        "cdd_palm_overlap_frac": _fmt_float(cdd_palm_overlap_frac, 4) if palm_span_iv else "",
        "cdd_palm_overlap_models": cdd_palm_overlap_models,
        "nonrdrp_neg_union_cov": _fmt_float(full_cov, 4) if all_intervals else "",
        "nonrdrp_neg_models": full_models,
        "nonrdrp_rdrpcatch_overlap_aa": str(int(rc_overlap_aa)) if rc_span_iv else "0",
        "nonrdrp_rdrpcatch_overlap_frac": _fmt_float(rc_overlap_frac, 4) if rc_span_iv else "",
        "nonrdrp_rdrpcatch_overlap_models": rc_overlap_models,
        "nonrdrp_palm_overlap_aa": str(int(palm_overlap_aa)) if palm_span_iv else "0",
        "nonrdrp_palm_overlap_frac": _fmt_float(palm_overlap_frac, 4) if palm_span_iv else "",
        "nonrdrp_palm_overlap_models": palm_overlap_models,
    }
...
def init_h5_columns() -> Dict[str, List[object]]:
    return {
        ...
        "cdd_rdrpcatch_overlap_aa": [],
        "cdd_rdrpcatch_overlap_frac": [],
        "cdd_palm_overlap_aa": [],
        "cdd_palm_overlap_frac": [],
        "nonrdrp_full_negative_sources": [],
        "nonrdrp_rdrpcatch_conflict_sources": [],
        "nonrdrp_palm_conflict_sources": [],
    }
...
def append_h5_record(cols: Dict[str, List[object]], label_rec: Dict[str, str]) -> None:
    ...
    cols["pfam_neg_qcov_max"].append(_coerce_float(label_rec.get("pfam_neg_qcov_max")))
    cols["cdd_neg_qcov_max"].append(_coerce_float(label_rec.get("cdd_neg_qcov_max")))
    ...
    cols["cdd_rdrpcatch_overlap_aa"].append(_coerce_float(label_rec.get("cdd_rdrpcatch_overlap_aa")))
    cols["cdd_rdrpcatch_overlap_frac"].append(_coerce_float(label_rec.get("cdd_rdrpcatch_overlap_frac")))
    cols["cdd_palm_overlap_aa"].append(_coerce_float(label_rec.get("cdd_palm_overlap_aa")))
    cols["cdd_palm_overlap_frac"].append(_coerce_float(label_rec.get("cdd_palm_overlap_frac")))
    cols["nonrdrp_full_negative_sources"].append(label_rec.get("nonrdrp_full_negative_sources", ""))
    cols["nonrdrp_rdrpcatch_conflict_sources"].append(label_rec.get("nonrdrp_rdrpcatch_conflict_sources", ""))
    cols["nonrdrp_palm_conflict_sources"].append(label_rec.get("nonrdrp_palm_conflict_sources", ""))
...
def write_compact_labels_h5(
    ...
    with h5py.File(out_h5, "w", libver="latest") as f:
        ...
        for name in ("chunk_id", "rps_pos_e_min", "rps_pos_models", "rps_neg_models", "span_start_aa", "span_end_aa", "decision_code", "decision_span_source", "nonrdrp_full_negative_sources", "nonrdrp_rdrpcatch_conflict_sources", "nonrdrp_palm_conflict_sources"):
        ...
        for name in (
            ...
            "palm_rdrp_score", "pfam_neg_qcov_max", "cdd_neg_qcov_max",
            "pfam_rdrpcatch_overlap_aa", "pfam_rdrpcatch_overlap_frac",
            "pfam_palm_overlap_aa", "pfam_palm_overlap_frac",
            "cdd_rdrpcatch_overlap_aa", "cdd_rdrpcatch_overlap_frac",
            "cdd_palm_overlap_aa", "cdd_palm_overlap_frac",
        ):
        ...
        f.attrs["source"] = "rdrpcatch+palm_annot+pfam(+cdd_optional)"
...
# -------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Label sequences using RdRpCATCH + palm_annot + Pfam/CDD negative conflict filtering")
    ...
    ap.add_argument("--rpsblast-fmt6", default="", help="optional CDD rpsblast fmt6(.gz) used for negative/unlabeled evidence only")
    ap.add_argument("--cdd-rdrps", default="", help="optional list of CDD domain IDs considered RdRP CDDs")
    ...
    ap.add_argument("--neg-ie", type=float, default=1e-5, help="max Pfam i-Evalue / CDD e-value for non-RdRP conflict domains")
    ap.add_argument("--neg-qcov", type=float, default=0.80, help="whole-sequence non-RdRP Pfam/CDD union coverage for FULL_NEG")
    ap.add_argument("--pfam-span-source", choices=["ali", "env"], default="ali", help="Pfam span source for coverage/overlap")

    ap.add_argument("--rc-ov-aa", type=int, default=50, help="min overlap aa for RdRpCATCH-vs-non-RdRP Pfam/CDD conflict")
    ...
    if bool(args.rpsblast_fmt6) != bool(args.cdd_rdrps):
        raise RuntimeError("Provide both --rpsblast-fmt6 and --cdd-rdrps together, or neither of them")

    rpsblast_hits: Dict[str, List[RpsBlastHit]] = {}
    cdd_rdrps: set = set()
    if args.rpsblast_fmt6:
        rpsblast_hits = parse_rpsblast_fmt6(args.rpsblast_fmt6, logger=log)
        cdd_rdrps = read_cdd_list(args.cdd_rdrps)
        if not cdd_rdrps:
            raise RuntimeError(f"Empty RdRP CDD list: {args.cdd_rdrps}")
        log.info("CDD negative evidence enabled with %d RdRP CDD IDs", len(cdd_rdrps))
    else:
        log.info("CDD negative evidence disabled")

    extra_rdrpcatch = set(rdrpcatch_hits) - set(qlens)
    extra_palm = set(palm_hits) - set(qlens)
    extra_dom = set(domtbl) - set(qlens)
    extra_rpsblast = set(rpsblast_hits) - set(qlens)
    if extra_rdrpcatch:
    ...
    if extra_rpsblast:
        log.warning("%d rpsblast IDs not found in FASTA will be ignored", len(extra_rpsblast))
    ...
    label_header = [
        ...
        "pfam_neg_qcov_max", "cdd_neg_qcov_max",
        "pfam_rdrpcatch_overlap_aa", "pfam_rdrpcatch_overlap_frac",
        "pfam_palm_overlap_aa", "pfam_palm_overlap_frac",
        "cdd_rdrpcatch_overlap_aa", "cdd_rdrpcatch_overlap_frac",
        "cdd_palm_overlap_aa", "cdd_palm_overlap_frac",
        "nonrdrp_full_negative_sources", "nonrdrp_rdrpcatch_conflict_sources", "nonrdrp_palm_conflict_sources",
    ]
    decision_header = [
        "chunk_id", "label", "decision_code", "L",
        "rdrpcatch_positive", "palm_positive", "has_usable_palm_span",
        "full_nonrdrp_negative", "full_nonrdrp_negative_sources",
        "pfam_full_nonrdrp_negative", "cdd_full_nonrdrp_negative",
        "rdrpcatch_overlap_conflict", "rdrpcatch_overlap_conflict_sources",
        "pfam_rdrpcatch_overlap_conflict", "cdd_rdrpcatch_overlap_conflict",
        "palm_overlap_conflict", "palm_overlap_conflict_sources",
        "pfam_palm_overlap_conflict", "cdd_palm_overlap_conflict",
        ...
        "cdd_negative_enabled", "cdd_neg_domain_count", "cdd_neg_union_cov", "cdd_neg_models",
        "cdd_rdrpcatch_overlap_aa", "cdd_rdrpcatch_overlap_frac", "cdd_rdrpcatch_overlap_models",
        "cdd_palm_overlap_aa", "cdd_palm_overlap_frac", "cdd_palm_overlap_models",
        "nonrdrp_neg_union_cov", "nonrdrp_neg_models",
        "nonrdrp_rdrpcatch_overlap_aa", "nonrdrp_rdrpcatch_overlap_frac", "nonrdrp_rdrpcatch_overlap_models",
        "nonrdrp_palm_overlap_aa", "nonrdrp_palm_overlap_frac", "nonrdrp_palm_overlap_models",
    ]
    ...
    with open_write_text_maybe_gzip(args.out) as out_lab, open_write_text_maybe_gzip(args.decision_out) as out_dec:
        ...
        for i, (qid, L) in enumerate(ordered_lengths, start=1):
            label_rec, decision_rec, decision_code = decide_record(
                ...
                cdd_hits=rpsblast_hits.get(qid, []),
                cdd_rdrps=cdd_rdrps,
                pos_ie=args.pos_ie,
    ...
    summary = {
        ...
        "rpsblast_fmt6": args.rpsblast_fmt6,
        "cdd_rdrps": args.cdd_rdrps,
        "parameters": {
            "pos_ie": args.pos_ie,
            "palm_score_min": args.palm_score_min,
            "neg_ie": args.neg_ie,
            "neg_qcov": args.neg_qcov,
            "pfam_span_source": args.pfam_span_source,
            "cdd_enabled": bool(args.rpsblast_fmt6),
            "rc_ov_aa": args.rc_ov_aa,
            "rc_ov_frac": args.rc_ov_frac,
            "palm_ov_aa": args.palm_ov_aa,
            "palm_ov_frac": args.palm_ov_frac,
        ...
        "counts": {
            ...
            "rpsblast_ids_in_fasta": sum(1 for qid, _ in ordered_lengths if qid in rpsblast_hits),
        },
        "outputs": {
            "label_tsv": args.out,
    ...
    if out_h5 is not None and h5_cols is not None:
        thresholds_meta = {
            "source": "rdrpcatch+palm_annot+pfam(+cdd_optional)",
            "pos_ie": args.pos_ie,
            "palm_score_min": args.palm_score_min,
            "neg_ie": args.neg_ie,
            "neg_qcov": args.neg_qcov,
